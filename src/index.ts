...
// Files are stored locally (no S3). They are NOT served via a public
// express.static mount anymore — that served any file in UPLOAD_DIR
// to anyone who knew its filename, forever, with zero access control,
// completely bypassing every permission check elsewhere in this app.
// Every file load now goes through /api/files/:fileKey, which
// requires a short-lived signed token generated at read time by an
// endpoint that has already run a real access check. See
// utils/fileAccessToken.ts and controllers/fileController.ts.
