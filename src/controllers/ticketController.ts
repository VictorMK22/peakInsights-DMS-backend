...
const nextTicketNumber = async (): Promise<string> => {
  const last = await Ticket.findOne().sort({ createdAt: -1 }).select("ticketNumber");
  const lastNum = last ? parseInt(last.ticketNumber.replace("TCK-", ""), 10) : 3000;
    ...
    const withSla = tickets.map((t) => {
      ...
      const slaHoursLeft =
        t.status === "resolved" || t.status === "closed"
          ? null
          : Math.round(((t.slaDueAt.getTime() - Date.now()) / 3600000) * 10) / 10;
      const resolutionTimeHrs = t.resolvedAt
        ? Math.round((((t.resolvedAt.getTime() - t.createdAt.getTime()) / 3600000) * 10)) / 10
        : undefined;
      return { ...obj, slaHoursLeft, resolutionTimeHrs };
    });

    res.json({ success: true, message: "Tickets retrieved", data: { tickets: withSla } });
  } catch (err) {
    next(err);
  }
};
...
export const createTicket = async (
  ...
  try {
    const { subject, description, department, category, priority, requester } = req.body as {
      subject: string;
      description?: string;
      department?: string;
      category?: string;
...
export const addTicketNote = async (
  ...
  try {
    ...
    const ticket = await Ticket.findByIdAndUpdate(
      req.params.id,
      { $push: { internalNotes: { note: note.trim(), by: req.user!.userId, at: new Date() } } },
      { new: true },
    );
    if (!ticket) {
      res.status(404).json({ success: false, message: "Ticket not found" });
